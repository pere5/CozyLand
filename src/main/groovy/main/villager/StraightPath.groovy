...
class StraightPath extends Action {
    Queue<DXY> path = new LinkedList<>()
    static Double STEP = 0.7
    int id

    StraightPath(DXY start, DXY dest, def nextTiles = null) {
        id = Model.getNewId()
        ...
        while (!closeEnough(nextStep, dest)) {
            ...
            nextStep = [px, py] as DXY
            path.add(nextStep)
        }
        path.add(dest)
    }

    private void testPrints(DXY start, DXY dest, def nextTiles) {
        ...
        def idx = Model.bresenham(pixelStart as IXY, pixelDest as IXY)
        (0..idx).each {
            def xy = Model.bufferedBresenhamResultArray[it] as IXY
            Model.model.drawables << new Artifact(parent: id, x: xy[0], y: xy[1])
        }

    }

    static boolean closeEnough(DXY pointA, DXY pointB) {
    ...
    static boolean closeEnoughTile(IXY tileA, IXY tileB) {
        int xBig = tileA[0] + 1
